<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="事件类型" prop="shijianleixing">
          <el-input v-model="ruleForm.shijianleixing" 
              placeholder="事件类型" clearable  :readonly="ro.shijianleixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `shijianleixingtongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shijianleixingtongji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shijianleixingtongjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
